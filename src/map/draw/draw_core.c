...
/*   Updated: 2025/03/09 20:53:15 by skang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "fdf.h"
...
void	draw_line(t_render_info *info, t_fdf start, t_fdf end)
{
	...
	while (1)
	{
		if (x + info->translate_x >= 0 && x + info->translate_x < WIN_WIDTH && y
			+ info->translate_y >= 0 && y + info->translate_y < WIN_HEIGHT)
			my_mlx_pixel_put(info->img, x + info->translate_x, y
				+ info->translate_y, start.color);
		else
			break ;
